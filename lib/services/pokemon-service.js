...
import { API_BASE, BATCH_SIZE, MAX_POKEDEX_ID } from '../../config/app.config.js';
import { FALLBACK_WEAKNESSES } from '../../constants/type-weaknesses.js';
import cache from './cache.js';

export async function fetchTotalCount() {
  const meta = await fetchJson(`${API_BASE}/pokemon?limit=1`);
  return Math.min(meta.count, MAX_POKEDEX_ID);
}

export async function fetchPokemonBatch(offset) {
  if (offset >= MAX_POKEDEX_ID) {
  ...
  const hasMore = !!data.next && offset + BATCH_SIZE < MAX_POKEDEX_ID;
